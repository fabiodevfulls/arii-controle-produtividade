...
export async function exportExcelReport(options: Options) {
  ...
  [...options.activities].sort((a, b) => a.occurredAt.localeCompare(b.occurredAt)).forEach((item) => {
  ...
  activitiesByDay.forEach((items, day) => {
    ...
    items.forEach((item, index) => {
      ...
      groupedDetailRows.push(`<row r="${groupedRow}" ht="22" customHeight="1">${xlsxCell(`A${groupedRow}`, item.userName, style)}${xlsxCell(`B${groupedRow}`, item.userEmail, style)}${xlsxCell(`C${groupedRow}`, item.kind === "protocol" ? "Protocolo" : "Ligação", style)}${xlsxCell(`D${groupedRow}`, item.distributionState || "-", style)}${xlsxCell(`E${groupedRow}`, item.protocol || "-", style)}${xlsxCell(`F${groupedRow}`, outcomeLabel(item), style)}${xlsxCell(`G${groupedRow}`, item.typologyName, style)}${xlsxCell(`H${groupedRow}`, item.quantity, style)}${xlsxCell(`I${groupedRow}`, dateLabel(item.occurredAt), style)}</row>`);
      groupedRow++;
    });
  });
  const summaryRows = [
  ...
  const summarySheet = sheet(summaryRows, `A1:I${lastSummaryRow}`, `<cols><col min="1" max="1" width="31" customWidth="1" style="10"/><col min="2" max="5" width="17" customWidth="1" style="10"/><col min="6" max="6" width="18" customWidth="1" style="10"/><col min="7" max="7" width="22" customWidth="1" style="10"/><col min="8" max="9" width="17" customWidth="1" style="10"/></cols>`, summaryMerges, true);
  const recordsSheet = sheet(recordHeader + recordRows, `A1:I${lastRecordRow}`, `<cols><col min="1" max="1" width="25" customWidth="1" style="10"/><col min="2" max="2" width="38" customWidth="1" style="10"/><col min="3" max="5" width="17" customWidth="1" style="10"/><col min="6" max="6" width="18" customWidth="1" style="10"/><col min="7" max="7" width="32" customWidth="1" style="10"/><col min="8" max="8" width="13" customWidth="1" style="10"/><col min="9" max="9" width="22" customWidth="1" style="10"/></cols>`, `<mergeCells count="1"><mergeCell ref="A1:I1"/></mergeCells>`, false, `<pane ySplit="3" topLeftCell="A4" activePane="bottomLeft" state="frozen"/>`, `A3:I${lastRecordRow}`);
  ...
  const files: Record<string, Uint8Array> = {
    ...
    "xl/styles.xml": strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="4"><font><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="22"/><name val="Aptos Display"/></font><font><color rgb="FFD6E3F2"/><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FF111827"/><sz val="11"/><name val="Aptos"/></font></fonts><fills count="8"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF071526"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF152B45"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF203A5A"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFAEEA18"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF5BF16"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFEEF4F8"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"><color rgb="FFD9E1E9"/></left><right style="thin"><color rgb="FFD9E1E9"/></right><top style="thin"><color rgb="FFD9E1E9"/></top><bottom style="thin"><color rgb="FFD9E1E9"/></bottom></border></borders><cellXfs count="11"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" applyAlignment="1"><alignment vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="2" borderId="0"/><xf numFmtId="0" fontId="2" fillId="3" borderId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="1" fontId="1" fillId="4" borderId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="1" fontId="3" fillId="5" borderId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="1" fillId="2" borderId="0"/><xf numFmtId="0" fontId="3" fillId="6" borderId="1"/><xf numFmtId="0" fontId="2" fillId="2" borderId="1" applyAlignment="1"><alignment vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="3" borderId="1" applyAlignment="1"><alignment vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="2" borderId="0"/></cellXfs></styleSheet>`),
  };
...
export function exportPdfReport(options: Options) {
  ...
  [...options.activities].sort((a, b) => a.occurredAt.localeCompare(b.occurredAt)).forEach((item) => { const day = item.occurredAt.slice(0, 10); pdfDays.set(day, [...(pdfDays.get(day) ?? []), item]); });
  ...
  const streams = pages.map(({ day, items }, page) => {
    ...
    const c = ["0.043 0.094 0.157 rg 0 0 595 842 re f", "0.949 0.718 0.020 rg 0 778 595 64 re f", `BT /F2 20 Tf 0.06 0.06 0.06 rg 34 807 Td (BACKOFFICE PRODUCAO | ${pdfText(options.title)}) Tj ET`, `BT /F1 9 Tf 0.12 0.18 0.25 rg 34 790 Td (${pdfText(options.subtitle)} | Gerado em ${pdfText(new Date().toLocaleString("pt-BR"))}) Tj ET`, "0.125 0.192 0.286 rg 34 716 527 45 re f", `BT /F1 8 Tf 0.68 0.76 0.86 rg 48 744 Td (QUANTIDADE DE PROTOCOLOS) Tj ET`, `BT /F2 18 Tf 1 1 1 rg 48 724 Td (${sum.protocols}) Tj ET`, "0.949 0.718 0.020 rg 34 680 527 26 re f", `BT /F2 7 Tf 0.06 0.06 0.06 rg 40 689 Td (REGISTROS${pdfText(dayLabel)}) Tj 105 0 Td (TIPO / UF) Tj 80 0 Td (PROTOCOLO) Tj 100 0 Td (TIPOLOGIA / PARECER) Tj 130 0 Td (DATA) Tj 80 0 Td (QTD) Tj ET`];
    items.forEach((item, i) => { const y = 660 - i * 28; if (i % 2 === 0) c.push(`0.067 0.133 0.216 rg 34 ${y - 7} 527 25 re f`); c.push(`BT /F1 7 Tf 0.9 0.94 0.98 rg 40 ${y} Td (${short(item.userName, 21)}) Tj 105 0 Td (${item.kind === "protocol" ? "Protocolo" : "Ligacao"} / ${pdfText(item.distributionState || "-")}) Tj 80 0 Td (${short(item.protocol || "-", 15)}) Tj 100 0 Td (${short(`${item.typologyName} | ${outcomeLabel(item)}`, 28)}) Tj 130 0 Td (${pdfText(new Date(item.occurredAt).toLocaleDateString("pt-BR"))}) Tj 80 0 Td (${item.quantity}) Tj ET`); });
