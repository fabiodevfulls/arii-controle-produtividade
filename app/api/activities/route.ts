  ...
  try {
    ...
    const result = await db
      .prepare(`INSERT INTO activities
        (user_email, kind, protocol, typology_id, typology_name, quantity, duration_seconds, occurred_at, distribution_state, backoffice_url, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`)
      .bind(
        currentUser.email,
        kind,
...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await db
      .prepare(`UPDATE activities SET kind = ?, protocol = ?, typology_id = ?, typology_name = ?,
